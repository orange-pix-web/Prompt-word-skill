...
export function referenceJsonToTemplate(input, defaults = {}) {
  ...
  for (const [providedKey, rawBox, index] of entries) {
    ...
    const binding = rawBox.binding || (rawBox.type === "副标题" ? "subtitle" : null);
    ...
    elements[key] = {
      type,
      label: String(rawBox.label || VISUAL_LABELS[key] || key),
...
export function serializeProductMarketing(entries) {
  const rows = entries
    ...
    .map((entry) => `| ${MARKETING_SCOPE_LABELS[entry.scope] || entry.scope} | ${entry.category === "*" ? "全部" : assertMarketingCell(entry.category)} | ${entry.product === "*" ? "全部" : assertMarketingCell(entry.product)} | ${assertMarketingCell(entry.region)} | ${assertMarketingCell(entry.text)} | ${Number(entry.priority) || 0} | ${entry.enabled === false ? "否" : "是"} |`)
    .join("\n");
  return `# 产品营销词配置

营销词按产品归属管理。生成时优先使用产品专属词，不足时依次使用分类通用词和全局通用词。

| 作用范围 | 分类 | 产品名称 | 位置类型 | 营销文案 | 优先级 | 启用 |
|---|---|---|---|---|---:|---|
${rows}
`;
}
...
function marketingScopeRank(entry, product) {
  if (entry.scope === "product" && entry.product === product.name) return 3;
  if (entry.scope === "category" && entry.category === product.category) return 2;
  if (entry.scope === "global") return 1;
  return 0;
}

function pickMarketing(entries, product, region, count, used) {
  if (count <= 0) return [];
  const matching = entries
    .filter((entry) => entry.enabled !== false && marketingScopeRank(entry, product) > 0)
    .filter((entry) => entry.region === region || entry.region === "不限位置")
...
export function resolveProductMarketing(entries, product, template) {
  ...
  const points = [];
  for (const region of regions) points.push(...pickMarketing(entries, product, region, 1, used));
  while (points.length < requestedCount) {
    const fallback = entries
      .filter((entry) => entry.enabled !== false && marketingScopeRank(entry, product) > 0)
      .filter((entry) => ["顶部卖点", "侧栏卖点", "底部卖点", "不限位置"].includes(entry.region))
      .sort((a, b) => marketingScopeRank(b, product) - marketingScopeRank(a, product) || (b.priority || 0) - (a.priority || 0))
  ...
  const subtitle = pickMarketing(entries, product, "副标题", 1, used)[0] || "";
  const support = pickMarketing(entries, product, "辅助文案", 1, used)[0] || subtitle;
  const footer = pickMarketing(entries, product, "底栏文案", 1, used)[0]
    || pickMarketing(entries, product, "底部卖点", 1, used)[0] || "";
