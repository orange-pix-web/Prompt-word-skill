...
export function generatePromptMarkdown({ product, templates, marketingRows }) {
  ...
  for (const entry of entries) {
    ...
    if (template.points > 0) {
      const selectedPoints = copy.points.slice(0, Math.min(3, template.points)).map((point) => `【${point}】`).join("");
    ...
    if (template.visualLayout?.elements) {
      ...
      const positions = Object.entries(template.visualLayout.elements)
        ...
        .map(([key, box]) => {
          const label = box.label || names[key] || key;
          const shape = box.shape && box.shape !== "none" ? `，使用${box.shape}形状` : "，不使用底板";
          const content = box.text ? `，内容要求“${box.text}”` : "";
          return `${label}位于画面左侧${Math.round(box.x)}%、顶部${Math.round(box.y)}%，宽约${Math.round(box.w)}%、高约${Math.round(box.h)}%，图层${Number(box.z || 1)}${shape}${content}`;
        })
        .join("；");
...
export function generateCombinedPromptMarkdown({ products, templates, marketingByCategory }) {
  ...
  for (const template of templates) {
    ...
    if (copy) {
      const marketingPoints = copy.points.slice(0, Math.min(3, template.points || 0)).map((item) => `【${item}】`);
    ...
    if (template.visualLayout?.elements) {
      const boxes = Object.entries(template.visualLayout.elements)
        ...
        .map(([key, box]) => {
          const label = box.label || key;
          const content = box.text ? `，内容要求“${box.text}”` : "";
          return `${label}：左侧${Math.round(box.x)}%、顶部${Math.round(box.y)}%、宽${Math.round(box.w)}%、高${Math.round(box.h)}%、形状${box.shape || "无"}、图层${box.z || 1}${content}`;
        });
