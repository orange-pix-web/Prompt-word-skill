...
export function normalizeMarketingGroupConfig(input = {}) {
  const requestedGroups = Array.isArray(input.groups) ? input.groups : MARKETING_COPY_GROUPS;
  const groups = [...new Set(requestedGroups
    .map((group) => String(group || "").trim())
    .filter(Boolean))];
  const safeGroups = groups.length ? groups : [...MARKETING_COPY_GROUPS];
  const aliases = Object.fromEntries(Object.entries(input.aliases || {})
    .map(([from, to]) => [String(from || "").trim(), String(to || "").trim()])
    .filter(([from, to]) => from && to));
  return { groups: safeGroups, aliases };
}

export function resolveMarketingGroup(value, config = {}) {
  const { groups, aliases } = normalizeMarketingGroupConfig(config);
  let resolved = String(value || "").trim();
  const visited = new Set();
  while (resolved && aliases[resolved] && !visited.has(resolved)) {
    visited.add(resolved);
    resolved = aliases[resolved];
  }
  return groups.includes(resolved) ? resolved : "";
}
...
export function parseProductMarketing(text = "", groupConfig = {}) {
  const rows = [];
  for (const [index, line] of text.split(/\r?\n/).entries()) {
    if (!line.trim().startsWith("|")) continue;
    const cells = line.trim().replace(/^\|/, "").replace(/\|$/, "").split("|").map((cell) => cell.trim());
  ...
  return rows.map((cells) => {
    ...
    entry.group = resolveMarketingGroup(entry.group, groupConfig)
      || resolveMarketingGroup(classifyMarketingGroup(entry), groupConfig)
      || classifyMarketingGroup(entry);
    return entry;
  });
}

export function serializeProductMarketing(entries, groupConfig = {}) {
  const rows = entries
    .slice()
    .sort((a, b) => (b.priority || 0) - (a.priority || 0) || a.text.localeCompare(b.text, "zh-CN"))
    .map((entry) => {
      const group = resolveMarketingGroup(entry.group, groupConfig)
        || resolveMarketingGroup(classifyMarketingGroup(entry), groupConfig)
        || classifyMarketingGroup(entry);
      return `| ${MARKETING_SCOPE_LABELS[entry.scope] || entry.scope} | ${entry.category === "*" ? "全部" : assertMarketingCell(entry.category)} | ${entry.product === "*" ? "全部" : assertMarketingCell(entry.product)} | ${assertMarketingCell(marketingRegions(entry).join("、"))} | ${assertMarketingCell(group)} | ${assertMarketingCell(entry.text)} | ${Number(entry.priority) || 0} | ${entry.enabled === false ? "否" : "是"} |`;
    })
    .join("\n");
  return `# 产品营销词配置
...
export function marketingJsonToEntries(input, defaults = {}, groupConfig = {}) {
  ...
  const entries = rawItems.map((raw, index) => {
    ...
    const requestedGroup = String(raw.group ?? raw.copyGroup ?? raw.semanticGroup ?? classifyMarketingGroup(probe)).trim();
    const group = resolveMarketingGroup(requestedGroup, groupConfig);
    if (!group) {
      throw new Error(`第${index + 1}条营销词分组“${requestedGroup}”不支持；请使用当前工作台可选分组名称`);
    }
    ...
    if (!regions.length || (regions.length === 1 && regions[0] === "不限位置" && !raw.position && !raw.region && !raw.regions && !raw.positions)) {
      regions = defaultRegionsForGroup(requestedGroup);
    }
  ...
  // AI 经常会在同一份识图结果中重复输出同一条文案。重复不是格式错误：
  // 保留第一条并把统计交给调用方展示，用户无需再手工清理 JSON。
  const seen = new Map();
  const uniqueEntries = [];
  ...
  Object.defineProperty(uniqueEntries, "importSummary", {
    value: { inputCount: entries.length, skippedDuplicateCount },
    enumerable: false,
  });
...
function latestVersionForLabel(files, productName, label) {
  const escaped = productName.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
  const escapedLabel = label.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
  const pattern = new RegExp(`^${escaped}-${escapedLabel}(?:-v(\\d+))?(?:-第\\d+组)?\\.md$`);
  return files
...
export function latestPromptVersion(files, productName) {
  return latestVersionForLabel(files, productName, "生图提示词");
}

export function promptFileName(productName, version, groupIndex = 1, groupTotal = 1, label = "生图提示词") {
  const versionSuffix = version === 1 ? "" : `-v${version}`;
  const groupSuffix = groupTotal > 1 ? `-第${String(groupIndex).padStart(2, "0")}组` : "";
  return `${productName}-${label}${versionSuffix}${groupSuffix}.md`;
}

export async function nextPromptPaths(folder, productName, groupTotal = 1, label = "生图提示词") {
  const files = await fs.readdir(folder);
  const latest = latestVersionForLabel(files, productName, label);
  const version = latest ? latest.version + 1 : 1;
  return Array.from({ length: groupTotal }, (_, index) => path.join(
    folder,
    promptFileName(productName, version, index + 1, groupTotal, label),
  ));
}

export async function nextPromptPath(folder, productName) {
  return (await nextPromptPaths(folder, productName))[0];
}
...
export function stripTemplateSourceDetails(text = "") {
  const cleaned = String(text)
    .replace(/(?:来源图|参考图)\s*[：:]?\s*【[^】]+】[^；。\n]*(?:[；。]|\n|$)/g, "")
    .replace(/(?:来源图|参考图)\s*[：:]\s*[^；。\n]*(?:[；。]|\n|$)/g, "")
    .replace(/(?:构图|布局)参考\s*[【\[][^】\]]+[】\]]\s*[；。]?\s*/g, "")
    .replace(/不得复制来源图中的/g, "不得复制模板原图中的")
    .replace(/仅用于构图、配色、文字层级和区域比例核对[；。]?/g, "")
    .replace(/^[\s；，、]+|[\s；，、]+$/g, "")
    .trim();
  return cleaned;
}

function genericTemplateName(text = "", knownProductNames = []) {
  let cleaned = stripTemplateSourceDetails(text);
  for (const productName of [...new Set(knownProductNames.map(String).filter(Boolean))]
    .sort((a, b) => b.length - a.length)) {
    cleaned = cleaned.replaceAll(productName, "");
  }
  cleaned = cleaned
    .replace(/^[\s·｜|：:、,，_—-]+|[\s·｜|：:、,，_—-]+$/g, "")
    .replace(/\s+/g, " ")
    .trim();
  return stripTemplateSceneName(cleaned);
}
...
function assertNoForeignProductNames(markdown, activeProductNames, knownProductNames) {
  ...
  const foreignNames = [...new Set((knownProductNames || []).map(String).filter(Boolean))]
    .filter((name) => !active.has(name)
      && ![...active].some((activeName) => activeName.includes(name))
...
export function generatePromptMarkdown({
  ...
  const displayName = String(product.displayName || name).trim() || name;
  const brand = String(product.brand || "").trim();
  const templateProductNames = [displayName];
  const activeProductNames = [...new Set([name, displayName])];
  const background = backgroundRule({
    backgroundMode,
    backgroundNote,
    productNames: templateProductNames,
    categories: [category],
  });
  ...
  const lines = [
    `# ${displayName}${brand ? `｜${brand}` : ""}｜网页版一次批量生图提示词`,
    "",
    `> 使用方法：网页版只上传【${imageName}】，复制下面代码块的全部内容，一次发送。`,
    "",
    "```text",
    `分别参照以下${entries.length}套提示词，使用我上传的产品【${displayName}】生成${entries.length}张比例1:1、尺寸1024×1024的中文电商主图，${entries.length}张图片分别单独发给我。不要九宫格，不要拼成一张，不要遗漏，各张构图必须明显不同。本次模板编号为【${numbers}】。`,
    "",
    "---",
    "",
    `# ${displayName} 电商主图统一约束`,
    "",
    `严格以上传的【${imageName}】为唯一产品参考。必须保持原包装容器或袋型、长宽比例、瓶盖、${brand ? `包装品牌【${brand}】、` : ""}包装颜色、产品名称【${displayName}】和净含量【${net}】不变。不得替换包装、修改品牌、拉伸产品或增加第二个产品，只允许等比例放大、缩小和轻微透视展示。`,
    `所有图片均为1:1正方形电商主图，高清商业摄影风格。产品完整清晰，通常占画面约40%至45%。主标题统一写产品名称【${displayName}】，必须是画面中最大、最醒目的粗体文字；副标题约为主标题一半大小；卖点文字的视觉高度约为副标题的80%，使用醒目粗体，不能生成成难以阅读的小字。`,
  ...
  for (const entry of entries) {
    ...
    const layout = contextualizeTemplateText(stripTemplateSourceDetails(rawLayout), templateProductNames, knownProductNames);
    const templateName = genericTemplateName(template.name, knownProductNames);
    lines.push("---", "", `# 提示词${template.number}｜${templateName}布局`, "");
    lines.push(`真实商业摄影风格，高清中文电商主图，正方形1:1比例。${layout}`);
    lines.push(background);
    lines.push(`画面中的【${displayName}】必须严格还原上传产品图，完整清晰、比例不变，作为画面主体。主标题使用画面最大粗体字写【${displayName}】。`);
    ...
    if (template.special && template.special !== "无") {
      const special = stripTemplateSourceDetails(template.special);
      if (special) lines.push(contextualizeTemplateText(special, templateProductNames, knownProductNames));
    }
...
export function generateBackgroundPromptMarkdown({
  templates,
  ...
  const background = backgroundMode === "custom"
    ? `本次临时背景要求（最高优先级）：${String(backgroundNote).replace(/[。；;，,\s]+$/g, "")}。如与模板原场景冲突，以本备注为准；只继承模板配色、环境层与非文字装饰。`
    : "本次保留模板原背景，只继承环境层、动物层、光影和非文字装饰；不要保留任何产品或文字占位。";
  const numbers = templates.map((template) => template.number).join("、");
  const lines = [
    "# 无产品无文案底图｜网页版一次批量生图提示词",
    "",
    "```text",
    `请分别参照以下${templates.length}套模板，生成${templates.length}张比例1:1、尺寸1024×1024的高清电商底图。${templates.length}张图片分别单独发给我，不要九宫格，不要拼图。本次模板编号为【${numbers}】。`,
    "",
    "# 无产品无文案底图统一约束",
    "只生成干净的背景底图：不得出现任何产品、包装、容器、袋子、瓶子、商标、logo、文字、数字、标题、副标题、卖点、净含量、底栏文案、水印、二维码、价格或人物手持商品。不得保留空白文字框、卖点底板、净含量框或产品占位框。",
    "保留模板的整体配色、光影、景深、环境氛围、动物背景、场景构图及非文字装饰形状；画面主体区域保持自然、干净，便于后续叠加产品和文案。",
    background,
    "",
  ];

  for (const template of templates) {
    let layout = stripTemplateSourceDetails(template.layout || "");
    for (const productName of [...new Set(knownProductNames.map(String).filter(Boolean))]
      .sort((a, b) => b.length - a.length)) {
      layout = layout.replaceAll(productName, "当前产品");
    }
    const templateName = genericTemplateName(template.name, knownProductNames);
    lines.push("---", "", `# 模板${template.number}｜${templateName}底图`, "");
    if (layout) lines.push(`参考该模板的背景气氛与场景构图：${layout}`);
    if (template.visualLayout?.elements) {
      const baseLayers = Object.entries(template.visualLayout.elements)
        .filter(([, box]) => box?.visible !== false && ["backgroundRegion", "animalRegion", "shape"].includes(box.type))
        .map(([key, box]) => visualBoxDescription(key, box, key));
      if (baseLayers.length) lines.push(`仅保留以下背景图层和非文字装饰：${baseLayers.join("；")}。`);
    }
    if (template.special && template.special !== "无") {
      let special = stripTemplateSourceDetails(template.special);
      for (const productName of [...new Set(knownProductNames.map(String).filter(Boolean))]
        .sort((a, b) => b.length - a.length)) {
        special = special.replaceAll(productName, "当前产品");
      }
      if (special) lines.push(`背景细节要求：${special}`);
    }
    lines.push("最终画面只保留背景和非文字装饰，不得生成任何包装、产品主体或可读文字。", "");
  }
  lines.push("---", "", `请按模板编号【${numbers}】依次生成${templates.length}张独立底图；每张均需明显不同，并严格执行无产品、无文字约束。`, "```", "");
  return lines.join("\n");
}

export function generateProductBackgroundPromptMarkdown({
  product,
  templates,
  backgroundMode = "product",
  backgroundNote = "",
  ...
  if (!product) throw new Error("请至少选择一个产品");
  if (!templates.length) throw new Error("请至少选择一个模板");
  const { name, imageName, category, net } = product;
  const displayName = String(product.displayName || name).trim() || name;
  const brand = String(product.brand || "").trim();
  const activeProductNames = [...new Set([name, displayName])];
  const background = backgroundRule({
  ...
  const numbers = templates.map((template) => template.number).join("、");
  const lines = [
    `# ${displayName}${brand ? `｜${brand}` : ""}｜无文案产品图提示词`,
    "",
    `> 使用方法：网页版只上传【${imageName}】，复制下面代码块的全部内容，一次发送。`,
    "",
    "```text",
    `分别参照以下${templates.length}套模板，使用我上传的产品【${displayName}】生成${templates.length}张比例1:1、尺寸1024×1024的高清电商产品背景图。${templates.length}张图片分别单独发给我，不要九宫格，不要拼图。本次模板编号为【${numbers}】。`,
    "",
    `严格以上传的【${imageName}】为唯一产品参考。产品包装必须完整清晰、比例不变；包装上的品牌${brand ? `【${brand}】、` : "、"}产品名称【${displayName}】、净含量【${net}】及所有原有印刷文字必须逐字保留，不得改写、遮挡、模糊或新增包装文字。`,
    "除产品包装原有印刷文字外，画面中不得出现任何文字、数字、Logo、水印、二维码、主标题、副标题、卖点、净含量标注、底栏文案、价格或促销信息。不得绘制空白文字框、卖点底板、净含量框或底栏通栏；原文字区域保持干净自然留白，方便后期自行添加文案。",
    "产品作为唯一前景主体；背景、动物和环境位于产品下方。保留模板的整体配色、光影、景深、环境氛围及非文字装饰形状，但不得把模板中的旧产品、旧文案或旧包装带入画面。",
    background,
    "",
  ];
  ...
  for (const template of templates) {
    const rawLayout = backgroundMode === "template" ? template.layout : stripTemplateSceneDetails(template.layout);
    const layout = contextualizeTemplateText(stripTemplateSourceDetails(rawLayout), [displayName], knownProductNames);
    const templateName = genericTemplateName(template.name, knownProductNames);
    lines.push("---", "", `# 模板${template.number}｜${templateName}产品背景图`, "");
    if (layout) lines.push(`沿用该模板的背景场景与产品构图关系：${layout}`);
    lines.push(`画面中的【${displayName}】必须严格还原上传产品图，作为唯一前景主体；包装本身所有文字完整保留。`);
    if (template.visualLayout?.elements) {
      const layers = Object.entries(template.visualLayout.elements)
        .filter(([, box]) => box?.visible !== false && ["product", "backgroundRegion", "animalRegion", "shape"].includes(box.type))
        .map(([key, box]) => visualBoxDescription(key, box, key));
      if (layers.length) lines.push(`仅参考以下产品与背景图层的位置：${layers.join("；")}。文字、卖点、净含量及底栏图层全部忽略并留白。`);
    }
    if (template.special && template.special !== "无") {
      const special = contextualizeTemplateText(stripTemplateSourceDetails(template.special), [displayName], knownProductNames);
      if (special) lines.push(`背景细节要求：${special}`);
    }
    lines.push("最终画面只包含产品包装及背景环境，不得生成任何额外文案或文字装饰。", "");
  }
  lines.push("---", "", `请按模板编号【${numbers}】依次生成${templates.length}张独立图片；输出前逐张核对：产品包装原字完整，画面额外文字为零。`, "```", "");
  const markdown = lines.join("\n");
  assertNoForeignProductNames(markdown, activeProductNames, knownProductNames);
  return markdown;
}
...
export function generateCombinedPromptMarkdown({
  ...
  const templateProductNames = [...new Set(products.map((product) => String(product.displayName || product.name).trim() || product.name))];
  const activeProductNames = [...new Set(products.flatMap((product) => [product.name, product.displayName || product.name]))];
  const background = backgroundRule({
    backgroundMode,
    backgroundNote,
    productNames: templateProductNames,
  ...
  const productNames = products.map((product) => `【${product.displayName || product.name}${product.brand ? `｜${product.brand}` : ""}】`).join("、");
