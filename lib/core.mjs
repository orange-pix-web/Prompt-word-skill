...
export function serializeProductMarketing(entries) {
  const rows = entries
    ...
    .map((entry) => `| ${MARKETING_SCOPE_LABELS[entry.scope] || entry.scope} | ${entry.category === "*" ? "全部" : assertMarketingCell(entry.category)} | ${entry.product === "*" ? "全部" : assertMarketingCell(entry.product)} | ${assertMarketingCell(marketingRegions(entry).join("、"))} | ${assertMarketingCell(entry.text)} | ${Number(entry.priority) || 0} | ${entry.enabled === false ? "否" : "是"} |`)
    .join("\n");
  return `# 产品营销词配置

营销词按产品归属管理。生成时优先使用产品专属词，不足时依次使用分类通用词和全局通用词。

| 作用范围 | 分类 | 产品名称 | 位置属性（可多个） | 营销文案 | 优先级 | 启用 |
|---|---|---|---|---|---:|---|
${rows}
`;
}
